...
app.get('/', (_req, res) => {
  res.json({ service: 'wai-dispute-resolver', status: 'ok', uptimeSec: Math.round(process.uptime()) });
});
  ...
  cronTask = cron.schedule('* * * * *', async () => {
    try {
      ...
      for (const invoice of disputes) {
        try {
          console.log(
            `[CRON ACTIVE] Outstanding Dispute Detected for ${invoice.clientName} (ID: ${invoice.invoiceId}). ` +
